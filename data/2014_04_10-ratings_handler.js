...
// run like this: $./ratings_handler.js ./ratings/ratings ./ratings/rates.data.crp
//
// files: output_name input_file.crp
//
// This is a special handler for Rankings dataset which I have recieved from Shaghayegh Sahebi.
// The files have 6 columns: objectID, subjectID, rating (0-10), flag, date of rating (in Unix epoch
// format), date of consumption (in 0000-00-00:1 format)
        ...
        if (tempData.length > 0) {
          ...
          if (firstLine) {
            ...
            for (var i = 0, j = 0; i < tempData.length; i++) {
              ...
              if (i == 5)
              {
                dummy = Date.parse(tempData[i].substring(0,10))/1000;
                if (dummy < 1)
                  dummy = NaN;
              }
              else
